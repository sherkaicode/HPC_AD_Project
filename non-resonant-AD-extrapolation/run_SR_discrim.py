...
parser.add_argument("-i","--indir",help="home folder",default="../working_dir")
...
def run_eval(set_1, set_2, code, save_dir, classifier_params, device, w_1 = None, w_2 = None, run_test = False, test_B = None, test_S = None, crop_weights = True):
    ...
    for i in range(int(args.classifier_runs)):
        ...
        NN.train(input_x_train, input_y_train, weights=input_w_train,  save_model=True, model_name = f"model_{local_id}" , n_epochs=classifier_params["n_epochs"], seed = i, outdir=save_dir, plot_loss=True)
